...
import com.bamf.settings.preferences.QuietHoursFragment;
...
public class NotificationManagerActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		boolean enableScreenRotation = SystemProperties.getBoolean(
				"lockscreen.rot_override", false)
				|| ((res.getBoolean(com.android.internal.R.bool.config_enableLockScreenRotation) && Settings.System
		...
		if (enableScreenRotation) {
			...
			if (res.getBoolean(com.android.internal.R.bool.config_enableLockScreenRotation)
					&& RotationPolicy.isRotationLocked(this)) {
		...
		mSwitch.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			...
			public void onCheckedChanged(CompoundButton buttonView,
				...
				if (mManager.findFragmentByTag("item") != null) {
					...
					Settings.System.putIntForUser(getContentResolver(),
							Settings.System.NOTIFICATION_MANAGER, isChecked ? 1
		...
		if (extras != null) {
			if (extras.getBoolean("quiet_hours", false)) {
				getFragmentManager().beginTransaction()
				.replace(R.id.container, new QuietHoursFragment(),"custom")
				.commit();
			}
		}
	...
	private void setupActionBar() {
		...
		mTitle = (TextView) actionBar.getCustomView().findViewById(
				android.R.id.title);
		mSwitch = (Switch) getActionBar().getCustomView().findViewById(
				R.id.switchWidget);
		mLabelCount = (TextView) getActionBar().getCustomView().findViewById(
				R.id.number_count);
	}
	...
	public void setupFragmentActionBar(int fragment, CharSequence title) {
		...
		case FRAGMENT_PREFERENCE:
			mTitle.setText((title == null) ? this
					.getText(R.string.notifications_main_title) : title);
		case FRAGMENT_ITEM:
			getActionBar()
			.setNavigationMode(ActionBar.NAVIGATION_MODE_STANDARD);
			showSwitchTitle(true);
	...
	public void onBackPressed() {
		if (mManager.findFragmentByTag("item") != null) {
			if (mManager.findFragmentByTag("item").isVisible()) {
				if (((NotificationItemFragment) mManager
						.findFragmentByTag("item")).showSaveDialog()) {
					super.onBackPressed();
				}
			}
		} else {
