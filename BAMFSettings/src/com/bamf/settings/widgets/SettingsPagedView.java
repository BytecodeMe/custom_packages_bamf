...
//    private View mSystemView2;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {    	
        ...
        int mPaddingTop = getPaddingTop();int mPaddingBottom = getPaddingBottom();
        int mPaddingLeft = getPaddingLeft();int mPaddingRight = getPaddingRight();
        ...
        if (tabHost != null) {
            ...
            if (tag != null) {
                ...
                if (currentPage >= mNumPerformancePages + mNumSystemPages &&
                        !tag.equals(tabHost.getTabTagForContentType(ContentType.Visual))) {
                    tabHost.setCurrentTabFromContent(ContentType.Visual);
                } else if (currentPage >= mNumPerformancePages && currentPage < (mNumPerformancePages + mNumSystemPages) &&
                		!tag.equals(tabHost.getTabTagForContentType(ContentType.System))) {
                	tabHost.setCurrentTabFromContent(ContentType.System);
                } else if (currentPage < mNumPerformancePages &&
                        !tag.equals(tabHost.getTabTagForContentType(ContentType.Performance))) {
...
//    		Toast.makeText(mContext, "Setting up Views", Toast.LENGTH_LONG).show();
    		...
    		if(mVisualView1 == null)
    			mVisualView1 = mLayoutInflater.inflate(R.layout.visual_settings_basic, null);
    		setupLayout(mVisualView1);
    		addView(mVisualView1);
//		if(mSystemView2 == null)
//			mSystemView2 = mLayoutInflater.inflate(R.layout.system_settings_maintenance, null);
//		setupLayout(mSystemView2);
//		addView(mSystemView2);
    		if(mSystemView1 == null)
    			mSystemView1 = mLayoutInflater.inflate(R.layout.system_settings_basic, null);
    		setupLayout(mSystemView1);
    		addView(mSystemView1);		
    		if(mPerformanceView1 == null)
    			mPerformanceView1 = mLayoutInflater.inflate(R.layout.performance_settings_kernel, null);
    		setupLayout(mPerformanceView1);
    		addView(mPerformanceView1);
    	}else{
//    		Toast.makeText(mContext, "Views aren't null", Toast.LENGTH_LONG).show();
    		return;
    	}
 ...
 // We want our pages to be z-ordered such that the further a page is to the left, the higher
    ...
    protected String getCurrentPageDescription() {
        ...
        } else if (page >= mNumPerformancePages && page < (mNumPerformancePages + mNumSystemPages)) {
    ...
    public void computeScroll() {
    	if (mScroller.computeScrollOffset()) {
        	int mScrollX = getScrollX();int mScrollY = getScrollY();
            ...
            AccessibilityManager acsm = (AccessibilityManager) getContext().getSystemService(Context.ACCESSIBILITY_SERVICE);
            if (acsm.isEnabled()) {
